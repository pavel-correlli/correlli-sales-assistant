...
# Clean UI: Metrics cards with light background
st.markdown("""
    <style>
    ...
    h3 { font-size: 28px !important; font-weight: 700 !important; margin-bottom: 20px !important; }
    </style>
...
if role == "CEO (Strategic Radar)":
    ...
    for market in active_markets:
        # LARGE MARKET TITLE
        st.markdown(f"### Market Dynamics: {market.upper()}")
        ...
        # 2. COMPARATIVE BAR CHART (RELATIVE LOGIC)
        ...
        chart_data = []
        # Total Calls (Relative Bar, Absolute Text)
        chart_data.append({"Period": "Reference", "Metric": "Total Calls", "Display": ref_total, "Scale": 100})
        chart_data.append({"Period": "Current", "Metric": "Total Calls", "Display": curr_total, "Scale": rel_scale})
        
        # Ratios
        for label, d, total in [("Reference", m_ref, ref_total), ("Current", m_curr, curr_total)]:
            v_pct = (d['vague_qty'].sum() / total * 100) if total > 0 else 0
            ni_pct = (d['not_interested_qty'].sum() / total * 100) if total > 0 else 0
            chart_data.append({"Period": label, "Metric": "Vague Ratio (%)", "Display": f"{v_pct:.1f}%", "Scale": v_pct})
            chart_data.append({"Period": label, "Metric": "Not Interested (%)", "Display": f"{ni_pct:.1f}%", "Scale": ni_pct})

        df_p = pd.DataFrame(chart_data)
        fig_bar = px.bar(df_p, x="Metric", y="Scale", color="Period", barmode="group",
                         text="Display", height=400, template="plotly_white",
                         color_discrete_map={"Current": "#4CAF50", "Reference": "#CED4DA"})
        fig_bar.update_layout(title_text="", yaxis_title="Scale (Ref = 100%)", showlegend=True)
        ...
        fig_line.update_layout(template="plotly_white", height=400, hovermode="x unified",
                              xaxis_title="Date", yaxis_title="Calls Quantity")
